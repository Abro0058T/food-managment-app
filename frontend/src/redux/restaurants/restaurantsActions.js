...
export const createOrderRestaurant = ({id,orderData}) => async (dispatch) => {
    try {
      dispatch({ type: CREATE_ORDER_RESTAURANT_REQUEST });
  
      // Replace with your MongoDB query to create the order
      const result = await axios.post(`${process.env.REACT_APP_BASE_URL_REST}/createOrders/${id}`, orderData);
  ...
  export const fetchCollectorHistory = (collectorId) => async (dispatch) => {
    try {
      ...
      const result = await axios.get(`${process.env.REACT_APP_BASE_URL_REST}/collectors/history/${collectorId}/history`);
